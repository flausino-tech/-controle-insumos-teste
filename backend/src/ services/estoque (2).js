...
// Perda "assumida": alguém escolheu explicitamente Saída/Perda ou Ajuste
// com motivo Perda/Quebra.
function isPerda(mov) {
  if (mov.tipo === 'SAIDA' && mov.finalidade === 'Perda') return true;
  if (mov.tipo === 'AJUSTE' && Number(mov.saldoResultante) < Number(mov.saldoAnterior)) return true;
  return false;
}

// Perda "invisível": qualquer ajuste que reduziu o saldo mas NÃO foi
// declarado como Perda/Quebra (ex.: contagem de inventário que bateu
// menor que o esperado, registrada só como "Inventário" ou "Correção").
// Sem isso, esse tipo de divergência nunca aparecia em nenhum relatório —
// o saldo só "encolhia" silenciosamente.
function isDivergenciaInventario(mov) {
  return mov.tipo === 'AJUSTE'
    && !['Perda', 'Quebra'].includes(mov.motivoAjuste)
    && Number(mov.saldoResultante) < Number(mov.saldoAnterior);
}
...
module.exports = { TIPOS, isPerda, isDivergenciaInventario, registrarMovimentacao, excluirMovimentacaoMaisRecente };
