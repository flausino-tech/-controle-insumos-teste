...
const verificarSenhaSchema = z.object({
  senha: z.string().min(1).max(200),
});

// Confere a senha do usuário logado sem alterar nada — usado como
// confirmação de segurança em ações sensíveis que não mexem no banco
// central (ex.: limpar os dados locais do Controle de Produção).
router.post('/verificar-senha', requireAuth, async (req, res) => {
  const parsed = verificarSenhaSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ erro: 'Informe a senha.' });
  const user = await prisma.user.findUnique({ where: { id: req.user.sub } });
  if (!user) return res.status(401).json({ erro: 'Sessão inválida.' });
...
const verificarGestorSchema = z.object({
  usuario: z.string().min(1).max(60),
  senha: z.string().min(1).max(200),
});

// Confirma credenciais de UM GESTOR (usuário + senha), independente de quem
// está logado no aparelho — usado no Controle de Produção pra autorizar
// ações que exigem supervisão (edição retroativa, dispensa de teste,
// reconhecimento de risco crítico, etc.). Diferente de /verificar-senha:
// aqui o alvo é escolhido no formulário, não a sessão atual, senão
// qualquer Operador logado no tablet "autorizaria" com a própria senha.
router.post('/verificar-gestor', requireAuth, async (req, res) => {
  const parsed = verificarGestorSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ erro: 'Informe usuário e senha do gestor.' });
  const { usuario, senha } = parsed.data;
  ...
  const user = await prisma.user.findUnique({ where: { usuario } });
  if (!user || !user.ativo || user.papel !== 'GESTOR') {
    registrarFalha(usuario);
    return res.status(401).json({ erro: 'Usuário ou senha de gestor incorretos.' });
  }
  const ok = await bcrypt.compare(senha, user.senhaHash);
  if (!ok) {
    registrarFalha(usuario);
  ...
  limparFalhas(usuario);
  res.json({ ok: true, usuario: user.usuario, nome: user.nome });
});
